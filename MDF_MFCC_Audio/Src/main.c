...
int main(void)
{
  ...
  /* Initialize MFCC processor (default sample rate 16 kHz) */
  MFCC_Init(16000);
  /* USER CODE END 2 */
...
/* DMA transfer callbacks: when half or full buffer is filled we process that half */
void HAL_DMA_XferHalfCpltCallback(DMA_HandleTypeDef *hdma)
{
  /* Process first half of RecBuff */
  MFCC_ProcessBuffer(&RecBuff[0], REC_BUFF_SIZE);
}

void HAL_DMA_XferCpltCallback(DMA_HandleTypeDef *hdma)
{
  /* Process second half of RecBuff */
  MFCC_ProcessBuffer(&RecBuff[REC_BUFF_SIZE], REC_BUFF_SIZE);
}
