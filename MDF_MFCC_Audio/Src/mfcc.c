 ...
 * Note: This is a simple, self-contained implementation intended as a starting point.
 * It uses CMSIS-RFFT for FFT, and basic mel/DCT transforms computed in C.
 */
...
static void build_mel_filters(void)
{
  ...
  for (int i = 0; i < MFCC_MEL_BANDS + 2; ++i)
  {
    ...
    /* store temporarily in dct_matrix row 0 to reuse - will reassign below */
    if (i < MFCC_MEL_BANDS + 2)
      ;
...
void MFCC_ProcessBuffer(const int16_t *pcm, uint32_t length)
{
  ...
  for (int off = 0; off + nfft <= (int)length; off += hop)
  {
    ...
    /* compute power spectrum (only bins 0..nbin-1) */
    ...
    /* DCT (type II) to get MFCC_COEFFS coefficients */
